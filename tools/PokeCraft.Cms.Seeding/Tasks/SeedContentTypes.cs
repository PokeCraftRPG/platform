...
using Logitar.CQRS;
using PokeCraft.Cms.Seeding.Models;
...
internal class SeedContentTypesTaskHandler : ICommandHandler<SeedContentTypesTask, Unit>
{
  ...
  public async Task<Unit> HandleAsync(SeedContentTypesTask task, CancellationToken cancellationToken)
  {
    string json = await File.ReadAllTextAsync("data/content_types.json", Encoding.UTF8, cancellationToken);
    IEnumerable<ContentTypePayload>? payloads = ToolsSerializer.Instance.Deserialize<IEnumerable<ContentTypePayload>>(json);
